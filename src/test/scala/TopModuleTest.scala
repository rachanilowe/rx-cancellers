...
class TopModuleTest extends AnyFreeSpec with ChiselScalatestTester {
  ...
  //       localNoise0 = (localNoise0 + Random.between(-16, 15)).max(-32).min(31)
  //       localNoise1 = (localNoise1 + Random.between(-16, 15)).max(-32).min(31)
  //       localNoise2 = (localNoise2 + Random.between(-16, 15)).max(-32).min(31)
  //       localNoise3 = (localNoise3 + Random.between(-16, 15)).max(-32).min(31)

  //       // val channelNoise = (Random.nextGaussian() * noiseAmplitude).round.toInt
  //       val receivedSignal = remoteSignal + (localNoise0) + (localNoise1 >> 4) + (localNoise2 >> 4) + (localNoise2 >> 4) // + channelNoise  // Scale local noise
