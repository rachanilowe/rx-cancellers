...
class HybridAdaptiveFIRFilter(val tapCount: Int, val segmentSize: Int, val gammaFactor: Int, val muFactor: Int) extends Module {
  ...
  val outputShifters = RegInit(VecInit(Seq.fill(numGroups - 1)(0.S(16.W))))
  ...
  val errorShifters = RegInit(VecInit(Seq.fill(numGroups - 1)(0.S(16.W))))
  ...
  for ((seg, idx) <- segments.zipWithIndex) {
    ...
    seg.io.partialSum := 0.S(16.W)
    seg.io.error := 0.S(16.W)
